...
def get_loggers():
    """Lazy load loggers with fallback."""
    if 'loggers' not in _cache:
        try:
            ...
            try:
                ...
                # Fallback to print
                _cache['loggers'] = (None, None)
    return _cache['loggers']

def get_database_models():
    """Lazy load database models with better error handling."""
    if 'db_models' not in _cache:
        try:
            ...
            api_logger, system_logger = get_loggers()
            if system_logger:
...
def get_s3_manager():
    """Lazy load S3 manager with better error handling."""
    if 's3_manager' not in _cache:
        try:
            ...
            if system_logger:
                ...
                print("✅ S3 manager loaded successfully")
        except ImportError as e:
...
def get_file_manager():
    """Lazy load file manager with better error handling."""
    if 'file_manager' not in _cache:
        try:
            ...
            if system_logger:
                ...
                print("✅ File manager loaded successfully")
        except ImportError as e:
...
def get_db_connection():
    ...
    if 'db_connection' not in _cache:
        try:
            # Try optimized connection pool first
            from src.database.connection_pool import get_pooled_db_connection
            _cache['db_connection'] = get_pooled_db_connection()
            api_logger, system_logger = get_loggers()
            if system_logger:
                system_logger.info("✅ Pooled database connection initialized")
            else:
                print("✅ Pooled database connection initialized")
        except ImportError:
            # Fallback to regular connection
            db_models, db_available = get_database_models()
...
if __name__ == "__main__":
    api_logger, system_logger = get_loggers()
    if system_logger:
        system_logger.info("🚀 Starting Full Authentication API server on port 8000...")
    else:
        print("🚀 Starting Full Authentication API server on port 8000...")
    uvicorn.run(
        "auth_server_full:app",
        host="0.0.0.0",
        port=8000,
        reload=True,
        log_level="info"
